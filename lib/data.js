...
/* ---------------------------------------------------------------
   Quizzes — answer keys and written-response marking.
   Admin-only, genuinely (RLS-enforced, not just UI-hidden):
   quiz_answer_keys and reading other people's quiz_submissions.
----------------------------------------------------------------- */

// Admin only — the real answer key (correct MC index, or model answers
// for written questions). Blocked for non-admins at the RLS level.
export async function fetchAnswerKey(quizId) {
  const { data, error } = await supabase.from("quiz_answer_keys").select("answers").eq("quiz_id", quizId).maybeSingle();
  if (error) throw error;
  return data?.answers || {};
}

export async function saveAnswerKey(quizId, topic, level, answers) {
  const { error } = await supabase.from("quiz_answer_keys").upsert(
    { quiz_id: quizId, topic, level, answers, updated_at: new Date().toISOString() },
    { onConflict: "quiz_id" }
  );
  if (error) throw error;
}

export async function deleteAnswerKey(quizId) {
  const { error } = await supabase.from("quiz_answer_keys").delete().eq("quiz_id", quizId);
  if (error) throw error;
}

// Secure multiple-choice grading — the answer key never reaches the
// browser before this call, and this call only returns the result.
export async function gradeQuiz(quizId, answers) {
  ...
  const body = await res.json();
  if (!res.ok) throw new Error(body.error || "Couldn't grade the quiz");
  return body;
}

// Written responses: staff submit their own; admins mark them.
export async function submitWrittenQuiz(quizId, topic, level, staffUserId, answers) {
  const { error } = await supabase.from("quiz_submissions").insert({
    quiz_id: quizId, topic, level, staff_user_id: staffUserId, answers,
  });
  if (error) throw error;
}

export async function fetchMySubmission(quizId) {
  const { data: { session } } = await supabase.auth.getSession();
  const userId = session?.user?.id;
  if (!userId) return null;
  const { data, error } = await supabase.from("quiz_submissions")
    .select("*").eq("quiz_id", quizId).eq("staff_user_id", userId)
    .order("submitted_at", { ascending: false }).limit(1).maybeSingle();
  if (error) throw error;
  return data;
}

// Admin only — every submission for a given quiz, to mark.
export async function fetchQuizSubmissions(quizId) {
  const { data, error } = await supabase.from("quiz_submissions")
    .select("*").eq("quiz_id", quizId).order("submitted_at", { ascending: false });
  if (error) throw error;
  return data || [];
}

export async function markSubmission(submissionId, patch, markedBy) {
  const row = { marked_by: markedBy, marked_at: new Date().toISOString() };
  if ("result" in patch) row.result = patch.result;
  if ("assessorNotes" in patch) row.assessor_notes = patch.assessorNotes;
  const { error } = await supabase.from("quiz_submissions").update(row).eq("id", submissionId);
  if (error) throw error;
}
...
// The "Lock & Assign" / "Recall" action — flips every module for this
// staff member at once, rather than toggling them one by one.
export async function setAllAssignedModulesUnlocked(staffUserId, unlocked) {
  const { error } = await supabase.from("assigned_modules")
