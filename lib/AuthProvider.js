...
export function AuthProvider({ children }) {
  ...
  const checkAdmin = useCallback(async (currentSession) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (error) console.error("Admin lookup failed:", error);
      ...
      console.error("Admin lookup threw:", error);
      setIsAdmin(false);
    } finally {
      setAdminChecked(true);
    }
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      // Seed the ref before any auth event arrives, otherwise the first
      // TOKEN_REFRESHED looks like a new user and re-gates the tree anyway.
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, newSession) => {
      // Auth broadcasts can arrive when this tab regains focus, when another
      // tab refreshes the session, and when Supabase replays INITIAL_SESSION.
      // If the identity is unchanged, these are credential updates rather than
      // a portal transition. Re-gating would briefly set `loading` true and
      // unmount the entire portal, losing the active domain and any nested
      // sub-domain state. Update the token in place and preserve the mounted
      // workspace for every same-user event.
