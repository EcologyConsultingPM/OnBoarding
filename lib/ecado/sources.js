...
export const SOURCES = {
  ...
  activities: source("project_activities", {
    ...
    // Work Activities are the authoritative delivery plan. The earlier mapping
    // referenced retired `assigned_to`, `survey_window_end` and `actual_date`
    // columns, which made the entire feed fail rather than return live work.
    // A planned due date is the actual delivery deadline where no separate
    // survey-window field exists; `completed_at` prevents completed rows from
    // being flagged as outstanding.
    dueDate: "due_date", assignedTo: "staff_user_id", surveyWindowEnd: "due_date",
    actualDate: "completed_at", startDate: "start_date", progressPercent: "progress_percent", detail: "detail",
  ...
  deliverables: source("project_deliverables", {
    id: "id", projectId: "project_id", name: "title", status: "status",
    // Deliverables do not yet have an assignee or a separate completion field.
    // Keep those values explicit as unavailable instead of making the query
    // fail; due-date oversight can still run from the live register.
    dueDate: "due_date", owner: null, percentComplete: null, clientCommitted: null,
  }, { requiredFor: ["deliverable oversight"] }),
  ...
  }, { requiredFor: ["team-allocation reference"] }),
  // A week-by-week workload is calculated from active Work Activities and
  // approved staff capacity profiles. It is not a separate database table:
  // the collector treats this as a read-only, derived feed so Ecado can assess
  // capacity from the same planned dates and hours that power both portals.
  capacity: source("project_activities + staff_capacity_profiles", {
    staffEmail: null, weekStarting: null, allocatedHours: null, capacityHours: null,
  }, { derived: true, requiredFor: ["weekly capacity oversight"] }),
  ...
  }, { requiredFor: ["competency expiry oversight"] }),
  quotes: source("quote_drafts", {
    id: "id", ref: "quote_number", name: "project_name", status: "status",
    dueDate: "client_required_by", owner: "assigned_to", urgency: "urgency",
    sentAt: "quote_sent_on", quoteSent: "quote_sent", transferredAt: "transferred_at",
  }, { requiredFor: ["quote and client-deadline oversight"] }),
};
