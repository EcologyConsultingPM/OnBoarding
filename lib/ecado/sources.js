...
export const SOURCES = {
  ...
  deliverables: {
    ...
    fields: {
      ...
      name: "title",
      owner: null, // no owner column on the deliverable itself — ownership lives on its linked activities
      dueDate: "due_date",
      status: "status",
      percentComplete: null, // not tracked directly; would need aggregating from linked activities' progress
      clientCommitted: null, // no distinct "committed vs internal milestone" flag in this schema yet
    },
    updatedAtColumn: "updated_at",
  ...
  correctiveActions: {
    ...
    fields: {
      ...
      dueDate: "corrective_due",
      status: "corrective_status", // "open" | "closed" | "none" — isOpen() in classify.js treats anything not closed/complete/etc. as open, so "open" correctly counts and "none"/"closed" don't
      owner: null, // no assignee column — ownership is implicit via the audited form's submitter, not tracked directly here
    },
    updatedAtColumn: "audited_at",
    filter: { corrective_status: "open" },
  ...
  // Minimal staff_certifications table built specifically to close this gap.
  certifications: {
    enabled: true,
    table: "staff_certifications",
    fields: {
      id: "id",
      staffEmail: "staff_user_id", // raw UUID column — resolved to an email automatically by collect.js's PERSON_FIELDS handling
