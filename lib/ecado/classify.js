...
const TERMINAL_STATUSES = {
  ...
  quotes: ["withdrawn", "transferred", "converted", "closed", "cancelled"],
};
...
/** R6 — Capacity: over-allocation and spare capacity from the derived weekly workload feed. */
function allocationRules(snap, t, now) {
  if (!feedAvailable(snap, "capacity")) return [];
  const horizonDays = 21;
  const byStaffWeek = new Map();

  for (const r of rowsOf(snap, "capacity")) {
...
/** R8 — Commercial/client: quote drafts nearing or past the client's required date. */
function quoteRules(snap, t, now) {
  if (!feedAvailable(snap, "quotes")) return [];
  const out = [];
  for (const r of rowsOf(snap, "quotes")) {
    if (!isOpen(r.status, "quotes") || r.transferredAt || truthy(r.quoteSent)) continue;
    const due = s(r.dueDate);
    const workingDays = workingDaysUntil(due, now);
    if (workingDays === null) continue;
    const ref = s(r.ref) || s(r.name) || s(r.id);
    const owner = s(r.owner) || null;
    const overdue = workingDays < 0;
    const urgent = ["urgent", "high", "critical"].includes(s(r.urgency).toLowerCase());
    const rating = overdue ? "high" : workingDays <= t.quote.mediumDecisionWithinDays ? (urgent ? "high" : "medium") : null;
    if (!rating) continue;
    out.push(
      make({
        ruleId: overdue ? "quote.deadline_overdue" : "quote.deadline_due",
        rating,
        level: levelFor(rating),
        domain: "commercial",
        title: overdue ? `Quote deadline overdue — ${ref}` : `Quote deadline in ${workingDays} working day(s) — ${ref}`,
        whatHappened: overdue
          ? `Quote draft ${ref} was required by ${formatAuDate(due)} and is ${Math.abs(workingDays)} working day(s) overdue. It is still marked ${s(r.status) || "open"}.`
          : `Quote draft ${ref} is required by ${formatAuDate(due)} and remains ${s(r.status) || "open"}.${urgent ? " It is marked urgent." : ""}`,
        whyItMatters: "A missed quote commitment damages the first client interaction and can turn a live opportunity into a lost project. Early visibility gives the team time to confirm scope, capacity and review before the promised date.",
        whatNext: owner ? `Confirm ${owner} has the scope, fee and review support needed to issue this quote on time.` : "Assign a quote owner and confirm scope, fee and review support today.",
        owner,
        sourceType: "quote",
        sourceId: s(r.id),
        sourceRef: ref,
        dueInDays: workingDays,
      }),
    );
  }
  return out;
...
/** R10 — Regulatory updates awaiting assessment. */
...
/** R11 — Data gaps are findings. A missing safety feed is a risk, not a blank section. */
...
export function classify(snapshot, thresholds, now = new Date()) {
  return sortFindings([
    ...
    ...quoteRules(snapshot, thresholds, now),
