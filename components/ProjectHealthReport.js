...
export default function ProjectHealthReport({ onManageProject, onOpenProjectTracker }) {
  ...
  return <section className="phr" aria-label="Project Health Report"><header className="phr-hero"><div><span><BarChart3 size={16} /> Portfolio control · live project tracker data</span><h1>Project Health Report</h1><p>Screen the delivery, financial and resource condition of every active Project Tracker. Budget remaining uses quoted role rates; estimated profitability uses the agreed 60% delivery-cost basis for each recorded role.</p></div><ProjectTrackerExport scope="portfolio" /></header>
