...
export default function RemoteTasks({
  ...
  const assign = async () => {
    ...
    if (!form.project.trim() || !form.task.trim())
      return setError("Project and task are required.");
    try {
  ...
  const act = async (id, body) => {
    setError("");
    try {
      const res = await authFetch("PATCH", "/api/remote-tasks", {
        id,
        ...body,
      });
      const data = await res.json();
  ...
  const withdraw = async (id) => {
    if (
      !window.confirm(
        "Withdraw this task? It will remain in the audit history.",
      )
    )
      return;
    try {
  ...
  const terminal = (status) =>
    ["complete", "declined", "withdrawn"].includes(status);
  ...
  const visibleTasks =
    !isAdmin && Array.isArray(staffStates)
      ? tasks.filter((task) => staffStates.includes(task.status))
      : tasks;

  return (
    <section
      className={`rt${compact ? " rt--compact" : ""}`}
      aria-label="Task briefs"
    >
      <div className="rt-head">
        <h2>
          <ClipboardList size={17} /> Task briefs
        </h2>
        {!isAdmin && showHome ? (
          <a className="workspace-home-link rt-home-link" href="/">
            Home
          </a>
        ) : null}
        {isAdmin && (
          ...
          <div className="rt-grid">
            ...
            {field(
              ...
              <input
                ...
                onChange={(event) =>
                  setForm({ ...form, budget_hours: event.target.value })
                }
                placeholder="e.g. 3.0"
              />,
            )}
            ...
            {field(
              ...
              <input
                ...
                onChange={(event) =>
                  setForm({ ...form, resources: event.target.value })
                }
                placeholder="Links to files or folders"
              />,
            )}
            {field(
              "Notes / questions",
              <textarea
                ...
                placeholder="Instructions, assumptions or issues"
              />,
            )}
          </div>
          <p className="rt-accept-note">
            <Clock size={13} /> The task remains awaiting acceptance and does
            not appear in the staff calendar until it is accepted.
          </p>
          <div className="rt-create-actions">
            <button className="rt-assign" onClick={assign}>
              <Send size={14} /> Send task brief
            </button>
            <button
              className="rt-cancel"
              onClick={() => {
      ...
      <div className="rt-list">
        {visibleTasks.length ? (
          visibleTasks.map((task) => {
            ...
            const overdue =
              task.due_date &&
              STAFF_VISIBLE_ACTIVE.includes(task.status) &&
              task.status !== "complete" &&
              new Date(task.due_date) < new Date(new Date().toDateString());
            const inCalendar =
              task.due_date && CALENDAR_ACTIVE_STATES.includes(task.status);
            ...
            return (
              <article key={task.id} className="rt-card">
                <div className="rt-card-top">
                  <div className="rt-card-main">
                    ...
                    <div className="rt-card-meta">
                      ...
                      {task.due_date ? (
                        <span className={overdue ? "rt-overdue" : ""}>
                          Due{" "}
                ...
                {task.decline_reason ? (
                  <div className="rt-line rt-decline-note">
                    <strong>Decline / reassignment request:</strong>{" "}
                    {task.decline_reason}
                  </div>
                ) : null}
                {task.staff_note ? (
                  <div className="rt-line rt-staff-note">
                    <strong>Staff update:</strong> {task.staff_note}
                  </div>
                ) : null}
                {task.review_note ? (
                  <div className="rt-line rt-review-note">
                    <strong>Review:</strong> {task.review_note}
                  </div>
                ) : null}
                {!isAdmin && inCalendar ? (
                  <div className="rt-calendar-note">
                    <CalendarCheck size={13} /> Added to your staff portal
                    calendar on the due date.
                  </div>
                ) : null}
                {!isAdmin && task.status === "awaiting_acceptance" ? (
                  <div className="rt-calendar-note pending">
                    <Clock size={13} /> Accept this task to add its due date to
                    your staff portal calendar.
                  </div>
                ) : null}
                    ...
                    <div className="rt-actions-open">
                      <textarea
                        ...
                        placeholder={
                          task.status === "awaiting_acceptance"
                            ? "Question, availability issue or reassignment request…"
                            : "Progress update or a question…"
                        }
                        value={draft.staff_note ?? task.staff_note ?? ""}
                        onChange={(event) =>
                          setDraft({ ...draft, staff_note: event.target.value })
                        }
                      />
                      <div className="rt-actions-row">
                        {task.status === "awaiting_acceptance" && (
                          ...
                          >
                            <CircleCheckBig size={12} /> Accept task
                          </button>
                        )}
                        {task.status === "awaiting_acceptance" && (
                          <button
                            className="rt-btn decline"
                            onClick={() =>
                              act(task.id, {
                                action: "decline",
                                decline_reason: draft.staff_note ?? "",
                              })
                            }
                          >
                            <CircleX size={12} /> Decline / reassign
                          </button>
                        )}
                        {task.status !== "awaiting_acceptance" && (
                          <button
                            className="rt-btn note"
                            onClick={() =>
                              act(task.id, {
                                staff_note: draft.staff_note ?? "",
                              })
                        ...
                        {STAFF_WORKABLE.includes(task.status) && (
                          <button
                ...
                {isAdmin &&
                  ...
                  (isOpen ? (
                    <div className="rt-actions-open">
                      <textarea
                        rows={2}
                        placeholder="Review note or revision instructions…"
                        value={draft.review_note ?? task.review_note ?? ""}
                        onChange={(event) =>
                          setDraft({
                            ...draft,
          ...
          <p className="rt-empty">
            {isAdmin
              ? "No tasks assigned yet. Use ‘Assign a task’ to create one."
              : Array.isArray(staffStates) &&
                  staffStates.includes("awaiting_acceptance")
                ? "No task briefs are awaiting your decision."
                : Array.isArray(staffStates)
                  ? "No accepted task briefs are active in My Projects."
                  : "No task briefs assigned to you right now."}
          </p>
        )}
      </div>
    </section>
