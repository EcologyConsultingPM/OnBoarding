    ...
    <div className="ld">
      ...
      <div className="ld-list">
        {nodes.length ? nodes.map((n) => {
          ...
          return (
            <button key={n.id} className="ld-node-view" onClick={() => openNode(n)} disabled={isItem}>
              <div className="ld-node-icon" style={{ background: "#eef3e4", color: "#2c6a34" }}><Icon size={18} /></div>
              <div className="ld-node-text">
                <div className="ld-node-title">{n.title}</div>
                <div className="ld-node-meta">{isItem ? (n.file_kind || "Open") : `${n.childCount} item${n.childCount === 1 ? "" : "s"}`}</div>
              </div>
              {!isItem && <ChevronRight size={18} className="ld-node-chev" />}
            </button>
          );
        }) : <p className="ld-empty"><BookOpen size={16} style={{ verticalAlign: "-3px", marginRight: 6 }} />Nothing here yet. Content appears once it's been approved and released.</p>}
      </div>
    </div>
  );
}
