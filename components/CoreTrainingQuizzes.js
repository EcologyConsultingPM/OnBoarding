...
export default function CoreTrainingQuizzes({ onBack }) {
  ...
  if (quiz) {
    ...
    return <section className="ctq-shell" aria-labelledby="core-quiz-heading">
      ...
      <header className="ctq-hero"><span>{quiz.careerLevel} · {quiz.code}</span><h2 id="core-quiz-heading">{quiz.title}</h2><p>For every question, select the most defensible response under the stated facts — not the response that should be rejected. This assessment supports learning and does not confer delegated authority or signatory approval.</p><div className="ctq-progress"><span style={{ width: `${Math.round((answered / quiz.questionCount) * 100)}%` }} /></div><small>{answered} of {quiz.questionCount} questions answered · Knowledge threshold {quiz.threshold.passMark}/{quiz.questionCount}</small></header>
      ...
      <div className="ctq-question-list">{quiz.questions.map((question) => {
        ...
        })}</div>{feedback ? <div className={`ctq-feedback ${feedback.correct ? "correct" : "incorrect"}`}><strong>{feedback.correct ? "Correct" : `Not quite — most defensible response: ${feedback.correctAnswer} — ${question.options.find((option) => option.key === feedback.correctAnswer)?.text || "Review the highlighted response."}`}</strong><p>{feedback.rationale}</p></div> : null}</div></article>;
      })}</div>
