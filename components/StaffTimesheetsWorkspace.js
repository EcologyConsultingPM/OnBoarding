...
export default function StaffTimesheetsWorkspace() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!projectsResponse.ok)
        throw new Error(
          projectsData.error || "Could not load your project allocations.",
        );
      const allocatedProjects = projectsData.projects || [];
      setProjects(allocatedProjects);

      const historyResponse = await fetch("/api/my-project-tracker", {
        headers: authHeaders(),
      });
      const historyData = await historyResponse.json();
      if (!historyResponse.ok)
        throw new Error(
          historyData.error || "Could not load your tracker history.",
        );
  ...
  const filteredEntries = useMemo(() => {
    ...
    const result = entries.filter((entry) => {
      ...
      const category =
        entry.activity_category || entry.category || entry.task_category || "";
      if (projectFilter !== "all" && entry.project_id !== projectFilter)
        return false;
      if (
        clientFilter !== "all" &&
        (entry.project_client || "") !== clientFilter
      )
        return false;
      ...
      if (
        startDate &&
        (!entryDate || String(entryDate).slice(0, 10) < startDate)
      )
        return false;
      if (endDate && (!entryDate || String(entryDate).slice(0, 10) > endDate))
        return false;
      if (
        search &&
        ![
          ...
          entry.note,
          entry.information,
        ].some((value) => normaliseText(value).includes(search))
      )
        return false;
      return true;
    });
    ...
    result.sort((a, b) => {
      if (sortBy === "updated_asc")
        return new Date(a.changed_at || 0) - new Date(b.changed_at || 0);
      if (sortBy === "project")
        return (
          String(a.project_name || "").localeCompare(
            String(b.project_name || ""),
          ) || new Date(b.changed_at || 0) - new Date(a.changed_at || 0)
        );
      if (sortBy === "client")
        return (
          String(a.project_client || "").localeCompare(
            String(b.project_client || ""),
          ) ||
          String(a.project_name || "").localeCompare(
            String(b.project_name || ""),
          )
      ...
      if (sortBy === "status")
        return (
          (STATUS[a.status]?.label || a.status).localeCompare(
  ...
  const totals = useMemo(
    () => ({
      ...
      blocked: entries.filter((entry) =>
        ["need_info", "paused_other"].includes(entry.status),
      ).length,
  ...
  const clients = useMemo(
    () =>
      [
        ...new Set(
          entries.map((entry) => entry.project_client).filter(Boolean),
        ),
  ...
  const exportXlsx = () => {
    const rows = filteredEntries.map((entry) => ({
      ...
      "Record type":
        entry.entry_type === "tracker_entry"
          ? "Project Tracker entry"
          : "Project activity update",
      "Recorded at": entry.changed_at
    ...
    const sheet = XLSX.utils.json_to_sheet(
      rows.length
        ? rows
        : [{ "Work date": "No entries match the selected filters." }],
    );
    sheet["!cols"] = [
      { wch: 16 },
      { wch: 26 },
    ...
    <main className="timesheet-page">
      <header className="timesheet-hero">
        ...
        <a
          ...
          href="https://staff.ecologyconsulting.au/"
          target="_blank"
          rel="noreferrer"
        >
      ...
      <section
        className="timesheet-summary"
        aria-label="Project tracker summary"
      >
        <div>
        ...
        <div>
          <strong className={totals.blocked ? "is-alert" : ""}>
            {totals.blocked}
          </strong>
      ...
      <section className="timesheet-tracker">
        ...
        <div
          ...
          aria-label="Filter project tracker history"
        >
        ...
        {error ? (
          <div className="timesheet-error" role="alert">
            {error}
          </div>
        ) : null}
        {!error && loading ? (
          <p className="timesheet-empty">
            Loading your project tracker history…
          </p>
        ) : null}
        {!error && !loading && filteredEntries.length === 0 ? (
          <p className="timesheet-empty">
            {entries.length
              ...
              : "No project tracker entries have been allocated to you yet."}
          </p>
        ) : null}
        {!error && !loading && filteredEntries.length > 0 ? (
                  ...
                  return (
                    <tr key={entry.id}>
                      <td>
                        ...
                        {entry.project_client ? (
                          <small>{entry.project_client}</small>
                        ) : null}
                      </td>
                      <td>
                        <strong>{entry.title}</strong>
                        {entry.task_category ? (
                          <small>{entry.task_category}</small>
                        ) : null}
                        {entry.allocation ? (
                          <small>{entry.allocation}</small>
                        ) : null}
                        {entry.notable_issues ? (
                          <small className="timesheet-reason">
                            Issue: {entry.notable_issues}
                          </small>
                        ) : entry.note ? (
                          <small className="timesheet-reason">
                            {entry.note}
                      ...
                      <td>
                        {entry.hours != null
                          ? `${entry.hours} h`
                          : entry.budget_hours != null
                            ? `${entry.budget_hours} h planned`
                            : "—"}
                      </td>
