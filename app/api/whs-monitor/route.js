...
export async function GET(request) {
  try {
    ...
    const [{ data: drafts }, { data: talks }, { data: incidents }, firstAidResult, { data: usersData }] = await Promise.all([
      ...
      access.admin.from("first_aid_kit_checks").select("id, checked_by, checked_by_name, check_date, selected_kits, restock_register, created_at, updated_at").order("created_at", { ascending: false }).limit(200),
      access.admin.auth.admin.listUsers(),
    ]);
    const firstAidChecks = firstAidResult.error ? [] : (firstAidResult.data || []);
    ...
    const awaiting = [
      ...
      ...firstAidChecks.filter((check) => (check.restock_register || []).length || (check.selected_kits || []).some((kit) => kit.outcome === "Failed — kit removed from service")).map((check) => ({ id: check.id, kind: "First aid kit check", title: (check.selected_kits || []).some((kit) => kit.outcome === "Failed — kit removed from service") ? "Kit removed from service" : `${(check.restock_register || []).length} restock / corrective action${(check.restock_register || []).length === 1 ? "" : "s"}`, author: check.checked_by_name || nameFrom(check.checked_by), updated_at: check.updated_at || check.created_at })),
    ...
    const bump = (id, key, approvedKey, isApproved) => {
      if (!people.has(id)) people.set(id, { email: nameFrom(id), drafts: 0, talks: 0, incidents: 0, firstAid: 0, approved: 0 });
    ...
    for (const check of firstAidChecks) bump(check.checked_by, "firstAid", null, !(check.restock_register || []).length && !(check.selected_kits || []).some((kit) => kit.outcome === "Failed — kit removed from service"));
    ...
    return Response.json({
      summary: {
        ...
        totalFirstAid: firstAidChecks.length,
        awaitingReview: awaiting.length,
      },
      byStatus: { drafts: draftCounts, talks: talkCounts, incidents: incidentCounts },
      awaiting,
      people: Array.from(people.values()).sort((a, b) => (b.drafts + b.talks + b.incidents + b.firstAid) - (a.drafts + a.talks + a.incidents + a.firstAid)),
    });
  } catch (error) {
