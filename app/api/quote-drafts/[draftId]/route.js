...
function readyForNumbers(values) {
  return Boolean(values.client_contacted && values.contact_date && values.contact_notes && values.scope_confirmed);
}
...
function toValues(body, directory, current) {
  ...
  const values = {
    ...
    development_type: text(body?.developmentType, 500),
    site_constraints: text(body?.siteConstraints, 5000),
    project_drivers: text(body?.projectDrivers, 3000),
    consent_authority: text(body?.consentAuthority, 500),
    authority_comments: text(body?.authorityComments, 5000),
    biodiversity_information_request: text(body?.biodiversityInformationRequest, 5000),
    ref_bar_requirements: text(body?.refBarRequirements, 5000),
    planning_pathway: text(body?.planningPathway, 1000),
