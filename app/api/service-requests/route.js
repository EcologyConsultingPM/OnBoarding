...
async function notifyAdministrators(admin, requestRecord, submittedBy, request) {
  try {
    ...
    await admin.from("portal_events").insert(recipientUsers.map((user) => ({ recipient_id: user.id, event_type: "service_request_submitted", severity: "action_required", title: `New ${TYPE_LABELS[requestRecord.request_type] || "service request"}`, body: `${submittedBy || "A staff member"} submitted: ${requestRecord.title}`, href: "/", source_table: "service_requests", source_id: requestRecord.id })));
