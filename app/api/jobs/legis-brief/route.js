...
async function generateBrief(weekOf, previousWatchlist, projectRoster, sourcePack) {
  ...
  if (!textBlocks.length) throw new Error("Model response contained no text content to parse.");
  // The model's final answer is its last text block — any earlier ones are
  // commentary alongside tool calls, not part of the structured JSON.
  const finalBlock = textBlocks[textBlocks.length - 1];
  const cleaned = finalBlock.replace(/^```json\s*/i, "").replace(/```\s*$/i, "").trim();
  ...
  try {
    ...
    if (!parsed) throw new Error(`Could not parse the briefing response as JSON: ${parseError.message}`);
  }
