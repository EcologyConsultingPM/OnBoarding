...
begin
  ...
  set
    project_number = coalesce(project_number, current_year || '-' || nextval('public.quote_draft_project_number_seq')::text),
    quote_number = coalesce(quote_number, 'Q-' || current_year || '-' || nextval('public.quote_draft_number_seq')::text),
    status = 'quote_drafting',
    updated_at = now()
  where id = p_draft_id
    and client_contacted = true
    and contact_date is not null
    and nullif(trim(coalesce(contact_notes, '')), '') is not null
    and nullif(trim(coalesce(scope_confirmed, '')), '') is not null;
